...
def _read_imageset_file(path):  #这个函数都变成int了从0到7481，但是我们标号是从122直接到10000，因为前两位按场景标的
...
def _calculate_num_points_in_gt(data_path,
    ...
    for info in mmcv.track_iter_progress(infos):
        ...
        indices = box_np_ops.points_in_rbbox(points_v[:, :3], gt_boxes_lidar)  ##(N,M),N是点个数，M是忽略“dontcare”框的个数 判断点是否在gt_boxes_lidar内，返回True或False值
        num_points_in_gt = indices.sum(0)
        num_ignored = len(annos['dimensions']) - num_obj
        num_points_in_gt = np.concatenate(
            [num_points_in_gt, -np.ones([num_ignored])])
        annos['num_points_in_gt'] = num_points_in_gt.astype(np.int32) ###(1,M+num_ignored)

        ####ci chu gai dong
        # assert len(annos['difficulty']) == len(annos['num_points_in_gt'])
        # for ind,num in enumerate(annos['num_points_in_gt']):
        #     if num >= 1:
        #         annos['difficulty'][ind] = 0
        #     else:
        #         annos['difficulty'][ind] = -1
...
def create_kitti_info_file(data_path,
    ...
    train_img_ids = _read_imageset_file(str(imageset_folder / 'train.txt'))##标号从122直接到10000，因为前两位按场景标的
    ...
    kitti_infos_train = get_kitti_image_info(  # 获取图片点云外参标注等信息返回列表，每个元素为单个样本所有信息的字典
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    kitti_infos_test = get_kitti_image_info(   # test有点不同
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
