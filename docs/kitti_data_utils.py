...
def get_label_anno(label_path):  ##整理成anno的dict
    annotations = {}
    annotations.update({
        'name': [],
        'truncated': [],
    ...
    num_gt = len(annotations['name']) #全部包含dontcare的
    ...
    if len(content) != 0 and len(content[0]) == 16:  # have score
        ...
        annotations['score'] = np.zeros((annotations['bbox'].shape[0], )) ##没分数设置为0
    index = list(range(num_objects)) + [-1] * (num_gt - num_objects) #[0,1,2,3,4]+[-1]*2=[0, 1, 2, 3, 4, -1, -1]
...
def get_kitti_image_info(path,
    ...
    if not isinstance(image_ids, list):
        image_ids = list(range(image_ids))  ###这个地方我们的image_ids是列表，标号从122直接到10000

    def map_func(idx):
        info = {}
        pc_info = {'num_features': 8}  ####由4改成8
        calib_info = {}
        ...
        if annotations is not None:
            ...
            add_difficulty_to_annos(info)  #  困难系数，需视情况而定，这里先按默认
        return info

    with futures.ThreadPoolExecutor(num_worker) as executor:  ##多线程读取处理
        image_infos = executor.map(map_func, image_ids)

    return list(image_infos)  #列表每个元素都是大的字典
...
def kitti_anno_to_label_file(annos, folder):  #暂时没找到地方用
    folder = Path(folder)
    for anno in annos:
...
def add_difficulty_to_annos(info):  # 根据图像内目标的高速和遮挡截断综合考虑
    min_height = [40, 25,
    ...
    max_trunc = [
        0.15, 0.3, 0.5
    ]  # maximum truncation level of the groundtruth used for evaluation 此处自己数据集可以修改困难类型
    annos = info['annos']
    ...
    for h, o, t in zip(height, occlusion, truncation):
        ...
        i += 1
    is_easy = easy_mask
    is_moderate = np.logical_xor(easy_mask, moderate_mask)  # 异或
